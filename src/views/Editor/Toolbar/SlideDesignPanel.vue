<template>
  <div class="slide-design-panel">
    ...
    <div class="row">
      <Select style="flex: 1;" :value="background.type"
        @update:value="value => updateBackgroundType(value as 'gradient' | 'image' | 'solid')" :options="[
      ...
      <Select style="flex: 1;" :value="background.imageSize || 'cover'"
        @update:value="value => updateBackground({ imageSize: value as 'repeat' | 'cover' | 'contain' })"
        v-else-if="background.type === 'image'" :options="[
          { label: '缩放', value: 'contain' },
          { label: '拼贴', value: 'repeat' },
          { label: '缩放铺满', value: 'cover' },
        ]" />

      <Select style="flex: 1;" :value="background.gradientType || ''"
        @update:value="value => updateBackground({ gradientType: value as 'linear' | 'radial' })" v-else :options="[
    ...
    <div class="background-gradient-wrapper" v-if="background.type === 'gradient'">
      ...
      <div class="row" v-if="background.gradientType === 'linear'">
        ...
        <Slider :min="0" :max="360" :step="15" :value="background.gradientRotate || 0"
          @update:value="value => updateBackground({ gradientRotate: value as number })" style="width: 60%;" />
      </div>
    </div>
    ...
    <div class="row">
      ...
      <Select style="width: 60%;" :value="viewportRatio" @update:value="value => updateViewportRatio(value as number)"
        :options="[
    ...
    <div class="row">
      ...
      <Select style="width: 60%;" :value="theme.fontName"
        @update:value="value => updateTheme({ fontName: value as string })" :options="[
          ...availableFonts,
          ...WEB_FONTS
        ]" />
    </div>
    ...
    <template v-if="moreThemeConfigsVisible">
      <div class="row">
        ...
        <Select style="width: 60%;" :value="theme.outline.style || ''"
      ...
      <div class="row">
        ...
        <Popover trigger="click" style="width: 60%;">
          <template #content>
            <ColorPicker :modelValue="theme.outline.color"
      ...
      <div class="row">
        ...
        <NumberInput :value="theme.outline.width || 0"
          @update:value="value => updateTheme({ outline: { ...theme.outline, width: value } })" style="width: 60%;" />
      </div>
      <div class="row" style="height: 30px;">
        <div style="width: 40%;">水平阴影：</div>
        <Slider style="width: 60%;" :min="-10" :max="10" :step="1" :value="theme.shadow.h"
          @update:value="value => updateTheme({ shadow: { ...theme.shadow, h: value as number } })" />
      </div>
      <div class="row" style="height: 30px;">
        <div style="width: 40%;">垂直阴影：</div>
        <Slider style="width: 60%;" :min="-10" :max="10" :step="1" :value="theme.shadow.v"
          @update:value="value => updateTheme({ shadow: { ...theme.shadow, v: value as number } })" />
      </div>
      <div class="row" style="height: 30px;">
        <div style="width: 40%;">模糊距离：</div>
        <Slider style="width: 60%;" :min="1" :max="20" :step="1" :value="theme.shadow.blur"
      ...
      <div class="row">
        ...
        <Popover trigger="click" style="width: 60%;">
          <template #content>
            <ColorPicker :modelValue="theme.shadow.color"
    ...
    <div class="theme-list">
      ...
      <div class="theme-item" v-for="(item, index) in PRESET_IMG_THEMES" :key="index" :style="{
        ...
        <div class="theme-item-content">
          <div class="text" :style="{ color: item.fontColor }">背景</div>
          <!-- <div class="colors">
            <div class="color-block" v-for="(color, index) in item.colors" :key="index" :style="{ backgroundColor: color}"></div>
          </div> -->

          <div class="btns">
            <div class="btn" @click="applyPresetThemeToSingleSlide(item, 'image')">应用</div>
            <div class="btn" @click="applyPresetThemeToAllSlides(item, 'image')">应用全局</div>
          </div>
        </div>
      </div>
    </div>
...
import { PRESET_THEMES, PRESET_IMG_THEMES } from '@/configs/theme'
