<template>
  <div class="canvas-tool">
    <div class="left-handler">
      ...
      <IconMoveOne class="handler-item" :class="{ 'active': showSelectPanel }" v-tooltip="'选择窗格'"
        @click="toggleSelectPanel()" />
      <IconSearch class="handler-item" :class="{ 'active': showSearchPanel }" v-tooltip="'查找/替换'"
    ...
    <div class="add-element-handler">
      ...
      <Popover trigger="click" v-model:value="shapePoolVisible">
        ...
        <IconGraphicDesign class="handler-item"
          :class="{ 'active': creatingCustomShape || creatingElement?.type === 'shape' }" v-tooltip="'插入形状'" />
      </Popover>
      ...
      <!-- <Popover trigger="click" v-model:value="mediaInputVisible">
        <template #content>
          <MediaInput @close="mediaInputVisible = false" @insertVideo="src => { createVideoElement(src); mediaInputVisible = false }"
            @insertAudio="src => { createAudioElement(src); mediaInputVisible = false }" />
        </template>
        <IconVideoTwo class="handler-item" v-tooltip="'插入音视频'" />
      </Popover> -->
      <VideoInput @change="files => insertVideoElement(files)">
        <IconVideoTwo class="handler-item" v-tooltip="'插入视频'" />
      </VideoInput>
    </div>
    ...
    <div class="right-handler">
      ...
      <Popover trigger="click" v-model:value="canvasScaleVisible">
        <template #content>
          <PopoverMenuItem center v-for="item in canvasScalePresetList" :key="item"
            @click="applyCanvasPresetScale(item)">{{ item }}%</PopoverMenuItem>
        </template>
...
const { creatingElement, creatingCustomShape, showSelectPanel, showSearchPanel } =
...
const insertVideoElement = async (files: FileList) => {
  const imageFile = files[0]

  if (!imageFile) return
  let fd = new FormData()
  fd.append("file", imageFile)
  const { data: res } = await axios.post("http://localhost:8080", fd)
  // console.log("这个数什么呢", res, files, imageFile)
  // const fileURL = URL.createObjectURL(imageFile)
  if (res.status == 1) createVideoElement(res.data.url)
