...
import { getUrlAsBase64} from '@/utils/image'
...
export default () => {
  ...
  // 设置幻灯片主题
  const setSlideTheme = (slide: Slide, theme: PresetTheme, type?: string) => {
    ...
    for (const el of slide.elements) {
      ...
      if (el.type === 'chart') {
        el.themeColor =
          [colorMap[tinycolor(el.themeColor[0]).toRgbString()]] || el.themeColor
  ...
  // 应用预置主题（单页）
  const applyPresetThemeToSingleSlide = async (theme: PresetTheme, type?: string) => {
    const imageUrl=await getUrlAsBase64(theme.background)
  ...
  // 应用预置主题（全部）
  const applyPresetThemeToAllSlides = async (theme: PresetTheme, type?: string) => {
    let imageUrl = ''
    let newTheme=theme
    if (type === 'image') { 
      imageUrl = await getUrlAsBase64(newTheme.background)
      newTheme.background=imageUrl
    }
    const newSlides: Slide[] = JSON.parse(JSON.stringify(slides.value))
    for (const slide of newSlides) {
      setSlideTheme(slide, newTheme, type)
    }
